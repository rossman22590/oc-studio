...
export const RobotExpressivePlayer = ({ position = [0, 0, 2], color = null, onPositionChange }: RobotExpressivePlayerProps) => {
  ...
  const lastManualInputAtRef = useRef(0);
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.code === "Digit1") triggerEmote("Jump");
      if (event.code === "Digit2") triggerEmote("Yes");
      if (event.code === "Digit3") triggerEmote("No");
      if (event.code === "Digit4") triggerEmote("Wave");
      if (event.code === "Digit5") triggerEmote("Punch");
      if (event.code === "Digit6") triggerEmote("ThumbsUp");

      if (event.code === "Space" && groundedRef.current) {
  ...
  useFrame((state, delta) => {
    ...
    const shouldAutopatrol =
      !manualDirectionalInput &&
      state.clock.elapsedTime - lastManualInputAtRef.current > AUTO_RESUME_DELAY &&
      groundedRef.current;
