...
const AgentStudioPage = () => {
  ...
  // Redirect to home if disconnected (but allow connecting/initializing state)
  const [hasInitialized, setHasInitialized] = useState(false);
  useEffect(() => {
    // Give connection a moment to initialize from localStorage
  ...
  useEffect(() => {
    // Only redirect if we've initialized and status is explicitly disconnected
    // Don't redirect if connecting or if we haven't initialized yet
    if (hasInitialized && status === "disconnected") {
      router.push("/");
    }
  ...
  /* ΓöÇΓöÇΓöÇ Dashboard: Activity feed ΓöÇΓöÇΓöÇ */
  ...
  /* ΓöÇΓöÇΓöÇ Dashboard: Total messages (approximation from agent output lines) ΓöÇΓöÇΓöÇ */
      ...
      if (newAgentId) {
        ...
        try {
          ...
          try {
            ...
            // Non-critical ΓÇö the route will create the dir on next File Manager load
          }
        } catch (err) {
          const message =
  ...
  // Kanban auto-completion ΓÇö always active, even when modal is closed
  ...
  return (
    <div className="relative min-h-screen w-screen overflow-hidden bg-background">
      ...
      <div className="relative z-10 flex h-screen flex-col gap-4 px-3 py-3 sm:px-4 sm:py-4 md:px-6 md:py-6">
        ...
        {showFleetLayout ? (
          <>
            {/* ΓöÇΓöÇΓöÇ Dashboard Stats Bar ΓöÇΓöÇΓöÇ */}
            ...
            {/* ΓöÇΓöÇΓöÇ Collapsible Fleet Sidebar ΓöÇΓöÇΓöÇ */}
            <div
          ...
          >
            {/* Expand sidebar button ΓÇô only visible on desktop when collapsed */}
            {sidebarCollapsed ? (
              <button
                type="button"
          ...
          {settingsAgent ? (
            ...
            {/* ΓöÇΓöÇΓöÇ Mobile Bottom Nav (replaces old inline toggle) ΓöÇΓöÇΓöÇ */}
            <MobileBottomNav
